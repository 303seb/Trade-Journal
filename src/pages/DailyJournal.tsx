      ...
      <div style={{
        ...
        <div style={{ padding: '20px 16px 14px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-label)', letterSpacing: '-0.01em' }}>Daily Journal</span>
          </div>
          <button
        ...
        <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Jump to Date</div>
          <input
            type="date"
            value={selectedDate}
            ...
            style={{
              ...
              padding: '6px 10px', fontSize: 12, color: 'var(--text-sub)', outline: 'none',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: savingTemplate || templates.length > 0 ? 6 : 0 }}>
            ...
            {!savingTemplate && (
              <button
                ...
                style={{ fontSize: 11, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', fontFamily: 'inherit', transition: 'color 0.15s' }}
          ...
          {savingTemplate && (
            <div style={{ display: 'flex', gap: 5, marginBottom: 6 }}>
              <input
                ...
                style={{ flex: 1, background: 'var(--bg-input)', border: '1px solid var(--border-mid)', borderRadius: 6, padding: '5px 8px', fontSize: 11, color: 'var(--text)', outline: 'none', fontFamily: 'inherit' }}
              />
              <button
                onClick={handleSaveTemplate}
                disabled={!templateName.trim() || !draft.trim()}
                style={{ background: 'var(--btn-bg)', color: 'var(--btn-text)', border: 'none', borderRadius: 6, padding: '5px 8px', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', opacity: templateName.trim() && draft.trim() ? 1 : 0.4 }}
              >✓</button>
              <button
                onClick={() => { setSavingTemplate(false); setTemplateName('') }}
                style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 7px', fontSize: 11, color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit' }}
              >✕</button>
            </div>
          )}
                ...
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12, color: 'var(--text-sub)', fontFamily: 'inherit',
                  ...
                  <button
                    ...
                    style={{ padding: '3px 5px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)', fontSize: 11, transition: 'color 0.15s', flexShrink: 0 }}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: isSelected ? 'var(--text)' : 'var(--text-sub)' }}>
                  ...
                  {preview && (
                    <div style={{ fontSize: 11, color: 'var(--text-dim)', lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 200 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          {draft.trim() && (
            <>
              <span style={{ fontSize: 12, color: 'var(--border-mid)' }}>·</span>
              <span style={{ fontSize: 12, color: '#4ade8055' }}>Auto-saved</span>
            </>
          )}
        </div>
